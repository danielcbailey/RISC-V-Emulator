...
var hoverInfoFormats = hoverInfoFormatsType{
	...
	srai:  "Shift Right Arithmetic Immediate Instruction.\n\nFormat: `srai <dst reg>, <src reg>, <amt>`\n\nExample: `srai x10, x11, 5` is the same as `x10 = x11 >> 5`\n\nNote that the immediate is a signed 6-bit value, so it must be between -32 and 31.\n\nNote, however, that this looks the same as `srli`, but the most-significant bit will be copied for each bit shifted.",
	...
	beq:  "Branch Equal Instruction.\n\nFormat: `beq <src reg 1>, <src reg 2>, <imm>`\n\nExample: `beq x10, x11, 40` is the same as `if x10 == x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",
	bne:  "Branch Not Equal Instruction.\n\nFormat: `bne <src reg 1>, <src reg 2>, <imm>`\n\nExample: `bne x10, x11, 40` is the same as `if x10 != x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",
	blt:  "Branch Less Than Instruction.\n\nFormat: `blt <src reg 1>, <src reg 2>, <imm>`\n\nExample: `blt x10, x11, 40` is the same as `if x10 < x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",
	bge:  "Branch Greater Than or Equal Instruction.\n\nFormat: `bge <src reg 1>, <src reg 2>, <imm>`\n\nExample: `bge x10, x11, 40` is the same as `if x10 >= x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",
	bltu: "Branch Less Than Unsigned Instruction.\n\nFormat: `bltu <src reg 1>, <src reg 2>, <imm>`\n\nExample: `bltu x10, x11, 40` is the same as `if x10 < x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",
	bgeu: "Branch Greater Than or Equal Unsigned Instruction.\n\nFormat: `bgeu <src reg 1>, <src reg 2>, <imm>`\n\nExample: `bgeu x10, x11, 40` is the same as `if x10 >= x11 { pc += 40/2 }`\n\nNote that the immediate is a signed 12-bit value, so it must be between -2048 and 2047.",

	jal:  "Jump and Link Instruction.\n\nFormat: `jal <imm>`\n\nExample: `jal 40` is the same as `x1 = pc; pc += 40/2`\n\nNote that the immediate is a signed 20-bit value, so it must be between -524288 and 524287.",
	...
	lui:   "Load Upper Immediate Instruction.\n\nFormat: `lui <dst reg>, <imm>`\n\nExample: `lui x10, 0x1234` is the same as `x10 = 0x1234000`\n\nNote that the immediate is an unsigned 20-bit value, so it must be between 0 and 1048575.",
	auipc: "Add Upper Immediate to PC Instruction.\n\nFormat: `auipc <dst reg>, <imm>`\n\nExample: `auipc x10, 0x1234` is the same as `x10 = pc + 0x1234000`\n\nNote that the immediate is an unsigned 20-bit value, so it must be between 0 and 1048575.",
